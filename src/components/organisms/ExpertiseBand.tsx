        ...
        {doubled.map((name, i) => (
          <li key={`${name}-${i}`} aria-hidden={i >= items.length} className="flex items-center gap-10 whitespace-nowrap">
            <span
              className={
                big
                  ? "font-display text-3xl font-semibold tracking-tight text-cream-50 sm:text-4xl"
                  : "text-sm font-medium uppercase tracking-[0.2em] text-slate-400"
              }
            >
              {name}
            </span>
            <svg viewBox="0 0 20 24" className={big ? "h-7 w-6" : "h-4 w-3"} aria-hidden>
              <path d="M12 3 V21 M3 6 L12 3 M4 18 L12 21" stroke="#b93538" strokeWidth="2.2" fill="none" />
              <path d="M12 5 H18 V19 H9" stroke="#4d4d4d" strokeWidth="1.6" fill="none" />
            </svg>
          </li>
        ))}
      </ul>
...
export function ExpertiseBand() {
  return (
    <section aria-label="Nos savoir-faire et notre secteur d'intervention" className="border-y border-white/5 bg-charcoal-950 py-10">
      <h2 className="sr-only">Nos savoir-faire</h2>
      <Row items={expertise} big />
      <div className="container mt-6 flex items-center gap-6">
        <p className="shrink-0 text-xs font-semibold uppercase tracking-[0.2em] text-brand-300">Nous intervenons à</p>
